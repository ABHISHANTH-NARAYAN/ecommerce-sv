...
Route::get('/product/{id}', [HomeController::class, 'show'])->name('product.show'); // <--- ADD THIS LINE HERE
/*
|--------------------------------------------------------------------------
| CART & WISHLIST
|--------------------------------------------------------------------------
...
Route::get('/dashboard', function () {
    ...
    $reviewChart = \App\Models\Review::selectRaw('MONTH(created_at) as month, COUNT(*) as total')
        ->groupBy('month')
        ->pluck('total', 'month')
        ->toArray();

    $enquiryChart = \App\Models\Enquiry::selectRaw('MONTH(created_at) as month, COUNT(*) as total')
        ->groupBy('month')
        ->pluck('total', 'month')
        ->toArray();

    $registrationChart = \App\Models\Registration::selectRaw('MONTH(created_at) as month, COUNT(*) as total')
        ->groupBy('month')
        ->pluck('total', 'month')
        ->toArray();
